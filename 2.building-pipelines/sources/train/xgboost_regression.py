...
- 훈련 및 검증 데이터 세트의 roc-rmse 값을 metrics_data 에 저장
    - 모델 훈련시 생성되는 지표(예: loss 등)는 크게 두가지 방식으로 사용 가능
        - 클라우드 워치에서 실시간으로 지표 확인
        - 하이퍼 파라미터 튜닝(HPO) 에서 평가 지표로 사용 (예: validation:roc-rmse)
...
Name                Regex
validation:auc	.*\[[0-9]+\].*#011validation-rmse:([-+]?[0-9]*\.?[0-9]+(?:[eE][-+]?[0-9]+)?).*
train:auc	    .*\[[0-9]+\].*#011train-rmse:([-+]?[0-9]*\.?[0-9]+(?:[eE][-+]?[0-9]+)?).*

실제 코드에 위의 Regex 형태로 print() 반영
print(f"[0]#011train-rmse:{train_rmse_mean}")
print(f"[1]#011validation-rmse:{validation_rmse_mean}")
...
def train_sagemaker(args):
    ...
    if os.environ.get('SM_CURRENT_HOST') is not None:
        args.train_data_path = os.environ.get('SM_CHANNEL_TR')
...
def main():
    ...
    data = pd.read_csv(f'{args.train_data_path}/abalone.csv')
    train = data.drop('Rings', axis=1)
    label = pd.DataFrame(data['Rings'])
...
#         print(f" train_auc_mean : {train_auc_mean}, train_auc_std : {train_auc_std}, test_auc_mean : {test_auc_mean}, test_auc_std : {test_auc_std}, ")
    ...
    metrics_data = {
        'classification_metrics': {
            'validation:auc': { 'value': cv_results.iloc[-1]['test-rmse-mean']},
            'train:auc': {'value': cv_results.iloc[-1]['train-rmse-mean']}
        }
    }
